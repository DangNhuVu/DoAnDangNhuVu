    ...
    <table>
        ...
        <?php
            ...
            foreach ($rows->fetchAll() as $row) {
                $sql_hang = "SELECT * from danhmuc where madm='" . $row['madm'] . "'";
                $sql_hang = $o->query($sql_hang)->fetchAll();
            ?>
        ...
        <form action="index.php?content=cart&action=update&idsp=<?php echo $row['idsp'] ?>" method="POST" name="update">
            <tr class="sanphamcart">
                ...
                <td><?php echo $sql_hang[0]['tendm'] ?></td>
                ...
                <td><input type="text" name="sl" value="<?php echo $_SESSION['cart'][$row['idsp']] ?>"
                        style="width:30px;" /></td>
                <td><?php echo number_format(($row['gia'] * ((100 - $row['khuyenmai1']) / 100)) * $_SESSION['cart'][$row['idsp']], 0, ",", ".") ?>
                </td>
                <td>
